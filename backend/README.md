...
REST API for Clinvia (formerly TBTrack), ported from the original Node/Express + `pg` backend (still available at `../backend-node-legacy` for reference) to Flask + Flask-SQLAlchemy. The API routes, request/response shapes, and JWT-based auth model are unchanged — the frontend requires no changes.

## Stack

- **Flask 3** — app factory pattern (`app/__init__.py`)
- **Flask-SQLAlchemy** — ORM models in `app/models.py`, mirroring the original `database/schema.sql` exactly
- **Flask-Migrate** (Alembic) — schema migrations live in `migrations/`
- **PyJWT** + **bcrypt** — same auth model as before: bearer JWTs, bcrypt-hashed passwords
- **psycopg3** — PostgreSQL driver (works with Render, Supabase, Neon, or local Postgres — anywhere a standard `postgresql://` connection string points)
...
│   └── routes/           # one blueprint per resource, mirrors the original Express routes
...
- `require_hospital` — restricts to `hospital`/`nurse`/`admin`/`viewer` roles (`hospital`/`nurse`/`viewer` still share identical permissions among themselves — see `../AUDIT.md`)
